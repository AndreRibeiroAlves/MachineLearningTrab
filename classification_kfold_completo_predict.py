...
def add_model(key, Model, models):
    
    if key in models:
        models[key][1].append(Model)
    else:
        models[key] = [key, [Model]]
...
split_mid = int((len(df)*70)/100)
df_crossval = df.iloc[:split_mid,:]
df_test = df.iloc[split_mid:,:]
...
for train, test in kf.split(X,y):
    ...
    add_model("Naive Bayes", model, models)
...
for model in models:
    ...
    for fold_model in fold_models:
        predicted = fold_model.predict(X)
        add_Score(key, y, predicted, Scores)
